...
object AlarmRuntimeTransition {
    ...
    private fun startRinging(current: AlarmRuntimeState?, event: AlarmRuntimeEvent, alarmId: Long): AlarmRuntimeState =
        when (current) {
    ...
    private fun snooze(current: AlarmRuntimeState?, event: AlarmRuntimeEvent, alarmId: Long): AlarmRuntimeState =
        when (current) {
            is AlarmRuntimeState.Ringing -> AlarmRuntimeState.Snoozed(alarmId)
            else -> illegal(current, event)
        }

    private fun dismiss(current: AlarmRuntimeState?, event: AlarmRuntimeEvent, alarmId: Long): AlarmRuntimeState =
        when (current) {
            is AlarmRuntimeState.Ringing -> AlarmRuntimeState.Dismissed(alarmId)
            else -> illegal(current, event)
        }

    private fun interrupt(current: AlarmRuntimeState?, event: AlarmRuntimeEvent, alarmId: Long): AlarmRuntimeState =
        when (current) {
